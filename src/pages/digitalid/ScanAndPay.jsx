...
const ScanAndPay = () => {
  ...
  const handleChange = (event) => {
    const selected = subAccountData.find(option => option.value === event.target.value)
  ...
  try {
    ...
    const res = await fetch(
      `${import.meta.env.VITE_BASE_URL}d/api/pay_with_digital/${data}/`,
      {
        method: 'POST',
        credentials: 'include',
        headers: {
