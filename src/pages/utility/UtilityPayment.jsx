...
const UtilityPayment = () => {
  ...
  const handleChange = (event) => {
    const selected = ServiceElectricityData.find(option => option.value === event.target.value);
    setService(selected)  
  }
...
const register = () => {
  ...
  const params = {
    ...
    serviceID: service.value,
    variation_code: type,
    amount: Math.round(totalAmountKobo / 100), // Convert kobo → Naira
    phone: phone,
    admin: userId
  };
...
<ContainerTitle title={'Utility Bill'}>
